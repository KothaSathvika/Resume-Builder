...
def test():  
    try:
        ...
        user_from_db = collection.find_one({"username": current_user})
        if user_from_db:
            return jsonify({"msg" : "Hi " + user_from_db['username']}), 200
        else:
...
def register():
    ...
    if (content_type == 'application/json' and request.method == "POST"):
        ...
        doc = collection.find_one({"username": new_user["username"]}) # check if user exist
...
def login():
    ...
    if (content_type == 'application/json' and request.method == "POST"):
        ...
        user_from_db = collection.find_one({'username': login_details['username']})  # search for user in database
        # If user exists
        if user_from_db:
            # Check if password is correct
            hashed_password = hashlib.sha256(login_details['password'].encode("utf-8")).hexdigest()
